...
import modules.youtube_analyzer
...
importlib.reload(modules.youtube_analyzer)
...
from modules.youtube_analyzer import download_youtube_video, analyze_youtube_video
...
app_mode = st.sidebar.selectbox(
    ...
    options=["🎥 Live Camera Training", "📺 YouTube Video Analyzer"],
    help="Select the workout tracker execution mode."
)

if app_mode == "🎥 Live Camera Training":
...
# Advanced Pose configuration (Visible in both modes)
st.sidebar.markdown("### 🏃 Pose Estimation Config")
with st.sidebar.expander("Pose Parameters", expanded=True if app_mode == "📺 YouTube Video Analyzer" else False):
...
if app_mode == "🎥 Live Camera Training":
    ...
    # ---------------------------------------------------------
    # Main Workspace Layout - YouTube Video Analyzer Mode
    # ---------------------------------------------------------
    st.markdown("### 📺 YouTube Video Pose Analyzer")
    st.write("Analyze custom YouTube workout videos by pasting their URL below. We will download the video, track pose landmarks on every frame, and plot a detection timeline.")
    ...
    if yt_url:
        ...
        if video_id:
            st.image(f"https://img.youtube.com/vi/{video_id}/maxresdefault.jpg", width=480, caption="Video Thumbnail")
            
        if st.button("Download & Analyze Video"):
            # Progress Placeholders
            download_status = st.empty()
            download_progress = st.empty()
            
            # Step 1: Download
            download_status.markdown("📥 **Downloading video from YouTube...**")
            progress_bar_dl = download_progress.progress(0.0)
            
            try:
                meta = download_youtube_video(
                    url=yt_url,
                    output_dir="videos/downloads",
                    progress_callback=progress_bar_dl.progress
                )
                download_status.success(f"✅ **Download complete:** {meta['title']}")
                ...
                analysis_status.markdown("🏃 **Running Pose landmarks detection frame-by-frame...**")
                progress_bar_pose = analysis_progress.progress(0.0)
                
                # Instantiate detector using sidebar pose parameters
                detector = PoseDetector(
                    model_complexity=model_complexity,
                    min_detection_confidence=min_det_conf,
                    min_tracking_confidence=min_track_conf
                ...
                input_file = meta['file_path']
                output_file = os.path.join("videos/downloads", f"processed_{os.path.basename(input_file)}")
                ...
                results = analyze_youtube_video(
                    ...
                    detector=detector,
                    progress_callback=progress_bar_pose.progress
                )
                
                analysis_status.success("✅ **Pose analysis complete!**")
                progress_bar_pose.empty()
                analysis_progress.empty()
                
                # Display Timeline
                st.markdown("#### 📈 Pose Detection Timeline")
                import pandas as pd
                df_timeline = pd.DataFrame(results['timeline_data'])
                if not df_timeline.empty:
                    st.line_chart(df_timeline.set_index("Timestamp (s)"))
                    
                # Save locally info
                st.markdown(f"💾 **Processed video saved locally at:** `{output_file}`")
                
                # Display analyzed video
                st.markdown("#### 🎥 Analyzed Video Playback")
                st.video(output_file)
                
            except Exception as e:
                st.error(f"Error during download or analysis: {e}")
